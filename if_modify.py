...
if __name__ == '__main__':
    ...
    with manager.connect(host=server, port=830, username=user, password=password, allow_agent=False, look_for_keys=False, hostkey_verify=False, unknown_host_cb=my_unknown_host_cb) as m:
        # Endless cycle
        while True:
            log = open(logfile, 'a')
            op_time = str(datetime.now())
